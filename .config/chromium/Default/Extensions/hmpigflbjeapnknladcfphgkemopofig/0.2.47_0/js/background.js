...
function executeUniFiCommand(params, port) {
    ...
    ubnt.sshCommander.doSshCmd(params.id, params.ip, params.username, params.password, params.port, command, finishCallback, progressCallback);
};
...
function executeSSHCommand(id, ip, user, password, command, port, callback) {
    if (knownDevices[ip]) {
        ubnt.sshCommander.doSshCmd(id, ip, user, password, 22, command, callback);
    } else {
        port.postMessage({
