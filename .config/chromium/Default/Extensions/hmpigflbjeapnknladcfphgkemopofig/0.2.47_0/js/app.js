...
function updateDevice (device) {
    ...
    for (i = 0; i < DEVICES.length; i ++) {
        ...
        if (item.mac === device.mac) {
            ...
            item.is_cloudkey = device.is_cloudkey || item.is_cloudkey;
            if (device.is_cloudkey) {
                item.display = device.display;
                item.webProtocol = 'https';
                item.webPort = 443;
            }
            item.short_ver = device.short_ver || item.short_ver;
            item.is_default = device.is_default || item.is_default;
            item.timestamp = device.timestamp || item.timestamp;
...
function updateDeviceRow (device) {
    function deviceLink () {
        if (device.family === 'UniFi' && !device.is_cloudkey) {
            return sanitize(device.ip);
        }
        var port,
            proto;

        port = device.webPort ? ':' + parseInt(device.webPort) : '';
        proto = device.webProtocol ? sanitize(device.webProtocol) + '://' : 'http://';
        return '<a href="' + proto + sanitize(device.ip) + port + '" target="_blank">' + sanitize(device.ip) + "</a>";
    };
    ...
    device.row.html(
        '<td width="15%">' + sanitize(deviceModel(device)) + '</td>' +
        '<td class="airos" width="20%">' + sanitize(device.hostname) + '</td>' +
        '<td width="10%">' + deviceLink(device) + '</td>' +
        '<td width="15%"><code>' + sanitize(device.formattedMac) + '</code></td>' +
        '<td width="15%">' + sanitize(deviceFirmware(device)) + '</td>' +
